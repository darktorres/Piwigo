...
async function sanitizeFilename(url: string): Promise<string> {
...
async function fetchHTMLContent(filePath: string): Promise<HTMLElement> {
  const content = await fs.readFile(filePath, "utf-8");
  const dom = new JSDOM(content);
  return dom.window.document.documentElement;
}

async function elementsAreEquivalent(elem1: Element, elem2: Element): Promise<boolean> {
...
async function compareHTMLFiles(filePath1: string, filePath2: string): Promise<boolean> {
