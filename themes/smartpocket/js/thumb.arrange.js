...
function SPThumbs(options) {
  ...
  $(window)
    .on("resize", function () {
      if (Math.abs(that.$thumbs.width() - that.prevContainerWidth) > 1) that.process();
    })
...
SPThumbs.prototype = {
  ...
  processLine: function (line, containerWidth, lastLine) {
    ...
    } else if (lastLine) rowHeight = Math.min(rowHeight, this.opts.rowHeight - this.opts.extraRowHeight);
    ...
    for (var i = 0; i < line.elements.length; i++) {
      ...
      else eltToRecover = Math.round((toRecover * eltW) / line.elementsWidth);
