  ...
  callService: function (method, parameters, options) {
    ...
    if (parameters) {
      for (var prop in parameters) {
        ...
        } else body += prop + "=" + encodeURIComponent(parameters[prop]) + "&";
      }
    }
    ...
    if (this.options.method == "POST") this.xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    try {
      this.xhr.send(body);
    } catch (e) {
  ...
  respondToReadyState: function (readyState) {
    ...
    if (readyState == 4 && xhr.status == 200) {
      ...
      if (resp != null) {
        ...
        else if (resp.stat == "ok") !this.options.onSuccess || this.options.onSuccess(resp.result);
