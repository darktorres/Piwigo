  ...
  <div class="card card-thumbnail {if isset($cat.path_ext)}path-ext-{$cat.path_ext}{/if} {if isset($cat.file_ext)}file-ext-{$cat.file_ext}{/if}">
      <div class="h-100">
        <a href="{$cat.URL}" class="ripple{if isset($smarty.cookies.view) and $smarty.cookies.view != 'list'} d-block{/if}">
          <img class="{if isset($smarty.cookies.view) and $smarty.cookies.view == 'list'}card-img-left{else}card-img-top{/if} thumb-img" src="{$derivative->get_url()}" loading="lazy" decoding="async" alt="{$cat.TN_ALT}" title="{$cat.NAME|@replace:'"':' '|@strip_tags:false} - {'display this album'|@translate}">        </a>
        <div class="card-body">
      ...
      <div class="card-body p-0">
        <a href="{$cat.URL}">
          <div class="media h-100">
            <img class="d-flex mr-3" src="{$derivative_square->get_url()}" loading="lazy" decoding="async" alt="{$cat.TN_ALT}">            <div class="media-body pt-2">
