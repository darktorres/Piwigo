...
class ThemeController
{
    ...
    public function assignConfig()
    {
        ...
        if (array_key_exists(
            'bootstrap_darkroom_navbar_main_style',
            $conf
    ...
    public function showUpgradeWarning()
    {
        global $page;
        $page['errors'][] = l10n(
            'Your selected color style has been reset to "bootstrap-darkroom". You can select a different color style in the admin section.'
        );
    }
    ...
    public function stripBreadcrumbs()
    {
        ...
        if (! empty($title)) {
            ...
            if ($page['section'] == 'recent_cats' || $page['section'] == 'favorites') {
                $title = preg_replace(
                    '/<\/a>([a-zA-Z0-9]+)/',
                    '</a><a class="nav-breadcrumb-item" href="' . make_index_url([
