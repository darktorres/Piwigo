...
#thumbnails>li{float:left;overflow:hidden;position:relative;margin-bottom:' . $vertical_margin . 'px;margin-left:' . $horizontal_margin . 'px}#thumbnails>li>a{position:absolute;border:0}'
    ...
    $template->scriptLoader->add(
        ...
        1,
        ['jquery'],
        'themes/' . $my_base_name . '/js/thumb.arrange.js'
    );
}
...
function modus_index_category_thumbnails($items): mixed
{
    ...
    if ($page['section'] != 'categories' || ! ($wh = (int) $conf['modus_theme']['album_thumb_size'])) {
        return $items;
    }
    ...
    if (isset($available_size)) {
        ...
        if ($available_size[2] > 1 && $selected_derivative) {
            ...
            if ($ratio_w > 1 || $ratio_h > 1) {
                if ($ratio_w > $ratio_h) {
                    $display_size = [
                        $available_size[0] / $available_size[2],
                        floor($size[1] / $ratio_w / $available_size[2]),
                    ];
                } else {
                    $display_size = [
                        floor($size[0] / $ratio_h / $available_size[2]),
                        $available_size[1] / $available_size[2],
                    ];
                }
            } else {
