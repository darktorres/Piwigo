...
function modus_smarty_prefilter($source)
{
    ...
    if (str_contains($source, '<div id="imageToolBar">') || str_contains(
        ...
        if ($pos !== false) {
            $source = substr_replace(
                $source,
                '<div class=actionButtonsWrapper><a id=imageActionsSwitch class=pwg-button><span class="pwg-icon pwg-icon-ellipsis"></span></a>{combine_script version=1 id=\'modus.async\' path="themes/`$themeconf.id`/js/modus.async.js" load=\'async\'}',
                $pos,
                0
            );
    ...
    if ((($pos = strpos($source, '<ul class="categoryActions">')) !== false || ($pos = strpos(
        $source,
        '<ul class=categoryActions>'
    )) !== false) && (($pos2 = strpos($source, '</ul>', $pos)) !== false
        && (substr_count($source, '<li>', $pos, $pos2 - $pos) > 2))) {
        $source = substr_replace(
            $source,
            '<a id=albumActionsSwitcher class=pwg-button><span class="pwg-icon pwg-icon-ellipsis"></span></a>{combine_script version=1 id=\'modus.async\' path="themes/`$themeconf.id`/js/modus.async.js" load=\'async\'}',
            $pos,
            0
        );
    }
