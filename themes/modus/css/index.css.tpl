...
.thumbnails {
	...
	list-style: none;
	text-align: center;
	/* to center the whole collection in .content */
}
...
.thumbnails .wrap1 {
	...
	/* Why 3 display option ??? */
	vertical-align: top;
	/* OK with Opera and IE6 not Geko */
	text-align: center;
	/* to center the thumbnail and legend in Geko/Opera */
}

.thumbnails .wrap2 {
	margin: 0;
	/* important reset the margins */
	display: table-cell;
	/* block prevents vertical-align here */
	vertical-align: middle;
	/* Ok with Opera and Geko not IE6 */
	border-radius: 4px;
...
.thumbnails .thumbLegend {
	overflow: hidden;
	/* oversized legend is clipped */
	height: 4em;
	/* legend height (don't set auto to be Gecko friendly)*/
}
...
{else}
	.overDesc {
		...
		width: 100%;
		left: 0;
		/* for IE7, all others use 0 by default*/
		bottom: -100px;
		overflow: hidden;
		text-align: center;
	...
	LI:hover>.overDesc {
		bottom: -1px;
		/* -1 for IE8 which has some rounding issues */
	}
...
.thumbnailCategories LI {
	...
	float: left;
	width: 49.7%;
	/* 49.7% for 2 per line, 33.2% for 3 per line*/
}
...
.albLegend {
	...
	filter: progid:DXImageTransform.Microsoft.gradient(startColorStr=#08000000, endColorStr=#90000000);
	/* IE to 9*/
	background-image: -moz-linear-gradient(top, rgba(0, 0, 0, 0.01), rgba(0, 0, 0, 0.5));
	/* FF 3.16 to 15 */
	background-image: -webkit-linear-gradient(top, rgba(0, 0, 0, 0.01), rgba(0, 0, 0, 0.5));
	/* Chrome, Safari */
	background-image: -ms-linear-gradient(top, rgba(0, 0, 0, 0.01), rgba(0, 0, 0, 0.5));
	/* IE ? to 9 */
	background-image: -o-linear-gradient(top, rgba(0, 0, 0, 0.01), rgba(0, 0, 0, 0.5));
	/* Opera 11 to 12 */
