...
include_once(PHPWG_ROOT_PATH . 'inc/functions_picture.inc.php');
            ...
            if (can_manage_comment('edit', $author_id)) {
                if (! empty($_POST['content'])) {
                    ...
                    switch ($comment_action) {
                        case 'moderate':
                            $_SESSION['page_infos'][] = l10n(
                                'An administrator must authorize your comment before it is visible.'
                            );
                            // no break
                        case 'validate':
...
$template->assign('display_info', $conf['picture_informations']);
...
if (isset($picture['next'])
    ...
    && ! str_contains((string) $_SERVER['HTTP_USER_AGENT'], 'Chrome/')) {
    $template->assign(
        'U_PREFETCH',
...
include(PHPWG_ROOT_PATH . 'inc/picture_rate.inc.php');
if ($conf['activate_comments']) {
    include(PHPWG_ROOT_PATH . 'inc/picture_comment.inc.php');
}

if ($metadata_showable && pwg_get_session_var('show_metadata') != null) {
    include(PHPWG_ROOT_PATH . 'inc/picture_metadata.inc.php');
}
