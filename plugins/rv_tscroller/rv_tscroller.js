...
var RVTS = {
  ...
  // Initialize the RVTS object
  init: function (params) {
    this.ajaxUrlModel = params.ajaxUrlModel;
    this.start = params.start;
    this.perPage = params.perPage;
    this.next = params.next;
    this.total = params.total;
    this.urlModel = params.urlModel;
    this.moreMsg = params.moreMsg;
    this.prevMsg = params.prevMsg;
    this.ajaxLoaderImage = params.ajaxLoaderImage;

    if (this.start > 0) {
      var $f = $(".navigationBar A[rel=first]");
      $("#thumbnails").before(`
        <div id=""rvtsUp"" style="text-align:center; font-size:120%; margin:10px">
          <a href="${$f.attr("href")}">${$f.html()}</a> | 
          <a href="javascript:RVTS.loadUp()">${this.prevMsg}</a>
        </div>
      `);
    }
  ...
  doAutoScroll: function () {
    ...
    $.ajax({
      ...
      success: function (htm) {
        ...
        //   RVTS.$thumbs.after(`
        //     <div style="text-align:center; font-size:180%; margin:0 0 20px">
        //       <a href="${RVTS.urlModel.replace("%start%", RVTS.next)}">
  ...
  engage: function () {
    ...
    RVTS.$thumbs.after(`
        <div id="ajaxLoader" style="display:none; position:fixed; bottom:32px; right:1%; z-index:999">
          <img src="${RVTS.ajaxLoaderImage}" width="128" height="15" alt="~">
        </div>
      `);
