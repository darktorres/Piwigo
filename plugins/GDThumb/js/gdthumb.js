var GDThumb = {
  ...
  setup: function(method, max_height, margin, do_merge, big_thumb, check_pv) {
    ...
    $(window).on("RVTS_loaded", function() { GDThumb.init(); });
    GDThumb.init();
  },
  ...
  init: function() {
    ...
    if(typeof mainlists !== 'undefined') {
      ...
      jQuery("ul.thumbnails .thumbLegend.overlay").on("click", function() { window.location.href = $(this).parent().find('a').attr('href'); });
      jQuery("ul.thumbnails .thumbLegend.overlay-ex").on("click", function() { window.location.href = $(this).parent().find('a').attr('href'); });
    }
  },
