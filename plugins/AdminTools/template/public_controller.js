var AdminTools = (function ($) {
  ...
  function populateMultiView() {
    ...
    var render = function (data) {
      ...
      $multiview.find('select[data-type="view_as"]').html(html).val(__this.multiView.view_as);

      html = "";
      $.each(data.themes, function (i, theme) {
        html += '<option value="' + theme + '">' + theme + "</option>";
      });
      $multiview.find('select[data-type="theme"]').html(html).val(__this.multiView.theme);

      html = "";
      $.each(data.languages, function (i, language) {
        html += '<option value="' + language.id + '">' + language.name + "</option>";
      });
      $multiview.find('select[data-type="lang"]').html(html).val(__this.multiView.lang);
