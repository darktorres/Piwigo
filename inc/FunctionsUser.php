...
FunctionsPlugins::add_event_handler(
    'try_log_user',
    \Piwigo\inc\FunctionsUser::class . '::pwg_login'
);
...
class FunctionsUser
{
    ...
    public static function validate_mail_address(
        ...
        if (($mail_address === null || $mail_address === '' || $mail_address === '0') && ! ($conf['obligatory_user_mail_address'] && in_array(
            script_basename(),
            ['register', 'profile']
        ))) {
            return '';
        ...
        if (defined('PHPWG_INSTALLED') && ($mail_address !== null && $mail_address !== '' && $mail_address !== '0')) {
            $query = '
SELECT count(*)
FROM ' . USERS_TABLE . '
...
;";
            ...
            $count = Mysqli::pwg_db_num_rows(Mysqli::pwg_query($query));
    ...
    FROM ' . USERS_TABLE . ';
  ');
        while ($r = Mysqli::pwg_db_fetch_assoc($q)) {
        ...
        if (count(
            $users_found
        ) != 1) { // If ambiguous, don't allow lowercase writing
            return $username;
    ...
    public static function register_user(
        ...
        $errors = FunctionsPlugins::trigger_change(
            'register_user_check',
            $errors,
            [
                'username' => $login,
        ...
        if (empty($errors)) {
            ...
            Mysqli::single_insert(USERS_TABLE, $insert);
            $user_id = Mysqli::pwg_db_insert_id();

            // Assign by default groups
            $query = '
SELECT id
  FROM ' . GROUPS_TABLE . '
  WHERE is_default = \'' . Mysqli::boolean_to_string(true) . '\'
  ORDER BY id ASC
;';
            ...
            if (count($inserts) != 0) {
                Mysqli::mass_inserts(USER_GROUP_TABLE, ['user_id', 'group_id'], $inserts);
            }

            $override = [];
            if ($conf['browser_language'] && ($language = self::get_browser_language())) {
    ...
    public static function build_user(
        ...
        if (! isset($user['theme_name']) || ! check_theme_installed(
            $user['theme']
        )) {
...
;';
        ...
        $result = Mysqli::pwg_query($query);
        $user_infos_row = Mysqli::pwg_db_fetch_assoc($result);
        ...
        if ($use_cache && (! isset($userdata['need_update']) || ! is_bool(
...
SELECT DISTINCT(id)
  ...
  WHERE category_id NOT IN (' . $userdata['forbidden_categories'] . ')
    AND level>' . $userdata['level'];
            $forbidden_ids = Mysqli::query2array($query, null, 'id');
            if ($forbidden_ids === []) {
                $forbidden_ids[] = 0;
            }
...
SELECT COUNT(DISTINCT(image_id)) as total
  ...
  WHERE category_id NOT IN (' . $userdata['forbidden_categories'] . ')
    AND image_id ' . $userdata['image_access_type'] . ' (' . $userdata['image_access_list'] . ')';
            [$userdata['nb_total_images']] = Mysqli::pwg_db_fetch_row(Mysqli::pwg_query($query));
            // now we update user cache categories
            $user_cache_cats = FunctionsCategory::get_computed_categories(
                $userdata,
                null
            );
            if (! self::is_admin(
                ...
                foreach ($user_cache_cats as $cat) {
                    if ($cat['count_images'] == 0) {
                        ...
                        FunctionsCategory::remove_computed_category($user_cache_cats, $cat);
                    }
                }
...
INSERT IGNORE INTO ' . USER_CACHE_TABLE . '
  ...
  VALUES
  (' . $userdata['id'] . ",'" . Mysqli::boolean_to_string($userdata['need_update']) . "',"
...
;';
        $authorizeds = Mysqli::query2array($query, null, 'image_id');
...
;';
        $favorites = Mysqli::query2array($query, null, 'image_id');
...
;';
        $private_array = Mysqli::query2array($query, null, 'id');
...
;';
        $authorized_array = Mysqli::query2array($query, null, 'cat_id');
...
;';
        $authorized_array =
          array_merge(
              $authorized_array,
              Mysqli::query2array($query, null, 'cat_id')
          );
...
SELECT id
  ...
  WHERE visible = \'false\'
;';
            $forbidden_array = array_merge($forbidden_array, Mysqli::query2array($query, null, 'id'));
...
;';
        ...
        [$user_id] = Mysqli::pwg_db_fetch_row($result);
        return $user_id;
    ...
    public static function get_userid_by_email(
        ...
        $email = Mysqli::pwg_db_real_escape_string($email);
...
;';
            ...
            if (Mysqli::pwg_db_num_rows($result) > 0) {
                $cache['default_user'] = Mysqli::pwg_db_fetch_assoc($result);
    ...
    public static function create_user_infos(
        ...
        if ($user_ids !== []) {
            ...
            foreach ($user_ids as $user_id) {
                ...
                $insert = array_merge(
                    array_map(Mysqli::pwg_db_real_escape_string(...), $default_user),
                    [
                        'user_id' => $user_id,
                        'status' => $status,
            ...
            Mysqli::mass_inserts(USER_INFOS_TABLE, array_keys($inserts[0]), $inserts);
        }
    }
...
WHERE ' . $conf['user_fields']['id'] . ' = ' . $user_id;
    ...
    public static function log_user(
        ...
        FunctionsPlugins::trigger_notify('user_login', $user['id']);
        ...
        if (isset($_COOKIE[$conf['remember_me_name']])) {
            ...
            if (count($cookie) === 3 && is_numeric(@$cookie[0]) && is_numeric(
                ...
                if ($key !== false && $key === $cookie[2]) {
                    ...
                    FunctionsPlugins::trigger_notify('login_success', stripslashes((string) $username));
                    return true;
                }
            }
    ...
    public static function try_log_user(
        ...
        return FunctionsPlugins::trigger_change(
            'try_log_user',
            false,
            $username,
            $password,
...
SELECT ' . $conf['user_fields']['id'] . ' AS id,
       ' . $conf['user_fields']['password'] . ' AS password
  FROM ' . USERS_TABLE . '
  WHERE ' . $conf['user_fields']['username'] . " = '" . Mysqli::pwg_db_real_escape_string($username) . '\'
;';
  ...
  SELECT ' . $conf['user_fields']['id'] . ' AS id,
         ' . $conf['user_fields']['password'] . ' AS password
    FROM ' . USERS_TABLE . '
    WHERE ' . $conf['user_fields']['email'] . " = '" . Mysqli::pwg_db_real_escape_string($username) . '\'
    ;';
...
;';
            $result = Mysqli::pwg_query($query);
            while ($user_infos_row = Mysqli::pwg_db_fetch_assoc($result)) {
                $status = $user_infos_row['status'];
            }

            if ($status != 'guest') {
                self::log_user($row['id'], $remember_me);
                FunctionsPlugins::trigger_notify('login_success', stripslashes($username));
                return true;
            }
        }

        FunctionsPlugins::trigger_notify('login_failure', stripslashes($username));
        return false;
    }
     ...
     * @param string $user_status used if $user not initialized
     */
    public static function is_autorize_status(
        $access_type,
        $user_status = ''
    ): bool {
    ...
    public static function get_recent_photos_sql(
        ...
        return $db_field . '>=LEAST('
          . Mysqli::pwg_db_get_recent_period_expression($user['recent_period'])
          . ',' . Mysqli::pwg_db_get_recent_period_expression(1, $user['last_photo_date']) . ')';
    }
...
;';
        ...
        FunctionsPlugins::trigger_notify('login_success', $key['username']);
...
SELECT
  ...
  WHERE user_id = ' . $user_id . '
;';
            $user_infos = Mysqli::query2array($query);
        ...
        $candidate = FunctionsSession::generate_key(30);
...
;';
        [$counter, $now, $expiration] = Mysqli::pwg_db_fetch_row(Mysqli::pwg_query($query));
        if ($counter == 0) {
            $key = [
                'auth_key' => $candidate,
                'user_id' => $user_id,
            ...
            Mysqli::single_insert(USER_AUTH_KEYS_TABLE, $key);

            $key['auth_key_id'] = Mysqli::pwg_db_insert_id();
...
';
    ...
    public static function userprefs_save(): void
    {
        ...
        $dbValue = Mysqli::pwg_db_real_escape_string(serialize($user['preferences']));
