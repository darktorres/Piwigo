...
(
  ...
  UNIQUE KEY the_key (' . implode(',', $dbfields['primary']) . ')
        ...
        if (($flags & MASS_UPDATES_SKIP_EMPTY) !== 0) {
            $func_set = static fn ($s) => sprintf('t1.%s = IFNULL(t2.%s, t1.%s)', $s, $s, $s);
        } else {
            $func_set = static fn ($s) => sprintf('t1.%s = t2.%s', $s, $s);
        }
...
UPDATE ' . protect_column_name($tablename) . ' AS t1, ' . $temporary_tablename . ' AS t2
  ...
  WHERE ' .
          implode(
              "\n    AND ",
              array_map(
                  static fn ($s) => sprintf('t1.%s = t2.%s', $s, $s),
                  $dbfields['primary']
              )
          );
        pwg_query($query);
...
INSERT ' . $ignore . ' INTO ' . protect_column_name($table_name) . '
  (' . implode(',', array_map('\Piwigo\inc\DbLayer\protect_column_name', $dbfields)) . ')
  VALUES';
                $first = false;
            } else {
                $query .= '
