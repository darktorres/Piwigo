...
class ScriptLoader
{
    ...
    private function fill_well_known(
        ...
        if (str_starts_with($id, 'jquery.')) {
            ...
            if (str_starts_with($id, 'jquery.ui.') && ($script->path === '' || $script->path === '0')) {
                $script->path = dirname((string) self::$known_paths['jquery.ui']) . sprintf('/%s.js', $id);
            }
