...
class functions
{
  ...
  ;';
    ...
    public static function cmpCat($a, $b)
    {
        if ($a['rank'] == $b['rank']) {
            return 0;
        }
  ...
  ;';
    ...
    public static function abs_fn_cmp($a, $b)
    {
        return abs($a) - abs($b);
    }
  ...
  ;';
    ...
    public static function TAT_tour_setup()
    {
        ...
        self::load_language('tour_' . $tour_name . '.lang', PHPWG_PLUGINS_PATH . 'TakeATour/', [
            'force_fallback' => 'en_UK',
        ]);
        ...
        if (in_array($tour_name, ['edit_photos', 'manage_albums', 'config', 'plugins'])) {
            ...
            self::load_language('tour_first_contact.lang', PHPWG_PLUGINS_PATH . 'TakeATour/', [
                'force_fallback' => 'en_UK',
            ]);
        }
        ...
        if (! empty($conf['mobile_theme']) && (self::get_device() != 'desktop' || self::mobile_theme())) {
            $template->assign([
                'TOGGLE_MOBILE_THEME_URL' => functions_url::add_url_params(htmlspecialchars($_SERVER['REQUEST_URI']), [
                    'mobile' => self::mobile_theme() ? 'false' : 'true',
                ]),
            ]);
        }
    }
  ...
  ;';
     ...
     */
    public static function load_profile_in_template($url_action, $url_redirect, $userdata, $template_prefixe = null)
    {
        global $template, $conf;
