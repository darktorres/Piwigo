...
functions_plugins::add_event_handler('try_log_user', '\Piwigo\inc\functions_user::pwg_login');
    ...
    public static function create_user_infos($user_ids, $override_values = null)
    {
        ...
        if (! empty($user_ids)) {
            ...
            foreach ($user_ids as $user_id) {
                ...
                $insert = array_merge(
                    array_map('\Piwigo\inc\dblayer\functions_mysqli::pwg_db_real_escape_string', $default_user),
                    [
                        'user_id' => $user_id,
                        'status' => $status,
  ...
  ;';
    ...
    public static function logout_user()
    {
        ...
        setcookie(
            ...
            '',
            0,
            ini_get('session.\Piwigo\inc\functions_cookie::cookie_path'),
            ini_get('session.cookie_domain')
        );
        setcookie($conf['remember_me_name'], '', 0, functions_cookie::cookie_path(), ini_get('session.cookie_domain'));
    }
     ...
     * @return bool
     */
    public static function is_autorize_status($access_type, $user_status = '')
    {
