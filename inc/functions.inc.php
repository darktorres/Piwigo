...
define(
    'MKGETDIR_DEFAULT',
    MKGETDIR_RECURSIVE | MKGETDIR_DIE_ON_ERROR | MKGETDIR_PROTECT_INDEX
);
...
function l10n(
    ...
    if (($val = ($lang[$key] ?? null)) === null) {
        if ($conf['debug_l10n'] && ! isset($lang[$key]) && ($key !== null && $key !== '' && $key !== '0')) {
...
function prepend_append_array_items(
    ...
    array_walk($array, function (&$value, $key) use (
        $prepend_str,
        $append_str
    ): void { $value = $prepend_str . $value . $append_str; });
    return $array;
}
...
function load_language(
    ...
    if (! empty($dirname) && ($filename !== '' && $filename !== '0') && ! ($options['return'] ?? null)
 ...
 * @param string $url
 */
function url_check_format($url): bool
{
    if (str_contains($url, '"')) {
        return false;
    }
