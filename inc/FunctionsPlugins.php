...
class FunctionsPlugins
{
    /**
     * Register an event handler.
     *
     * @param string $event the name of the event to listen to
     * @param callable $func the callback public static function
    ...
    public static function add_event_handler(
        ...
        if (isset($pwg_event_handlers[$event][$priority])) {
            foreach ($pwg_event_handlers[$event][$priority] as $handler) {
                if ($handler['public static function'] == $func) {
                    return false;
                }
            }
        }
    ...
    public static function remove_event_handler(
        ...
        for ($i = 0; $i < $counter; $i++) {
            if ($pwg_event_handlers[$event][$priority][$i]['public static function'] == $func) {
    ...
    public static function trigger_change(
        ...
        foreach ($pwg_event_handlers[$event] as $priority => $handlers) {
            foreach ($handlers as $handler) {
                ...
                $data = call_user_func_array($handler['public static function'], $args);
            }
        }
    ...
    public static function trigger_notify(
        ...
        foreach ($pwg_event_handlers[$event] as $priority => $handlers) {
            foreach ($handlers as $handler) {
                ...
                call_user_func_array($handler['public static function'], $args);
            }
        }
    }
...
;';
    ...
    public static function load_plugins(): void
    {
        ...
        if ($conf['enable_plugins']) {
            ...
            foreach ($plugins as $plugin) {// include main from a public static function to avoid using same public static function context
                self::load_plugin(
                    $plugin
                );
            }
