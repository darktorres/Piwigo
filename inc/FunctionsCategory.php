...
(id_uppercat is NULL';
  ...
  ' . FunctionsUser::get_sql_condition_FandF(
        ...
        $where = FunctionsPlugins::trigger_change(
...
SELECT *
  ...
  WHERE id = ' . $id . '
;';
        $cat = Mysqli::pwg_db_fetch_assoc(Mysqli::pwg_query($query));
        if (empty($cat)) {
            return [];
        }
        ...
        foreach ($cat as $k => $v) {
            ...
            if ($cat[$k] == 'true' || $cat[$k] == 'false') {
                $cat[$k] = Mysqli::get_boolean($cat[$k]);
            }
        }
  ...
  ;';
            $names = Mysqli::query2array($query, 'id');
    ...
    public static function get_category_preferred_image_orders()
    {
        ...
        return FunctionsPlugins::trigger_change('get_category_preferred_image_orders', [
            ...
            [l10n('Permissions'),                    'level DESC',           FunctionsUser::is_admin()],
...
SELECT id, permalink, 0 AS is_old
  ...
  WHERE permalink IN (' . $in . ')
;';
        $perma_hash = Mysqli::query2array($query, 'permalink');
    ...
    public static function get_display_images_count(
        ...
        if ($cat_count_images > 0) {
            if ($cat_nb_images > 0 && $cat_nb_images < $cat_count_images) {
                $display_text .= self::get_display_images_count(
                    $cat_nb_images,
                    $cat_nb_images,
                    0,
                    $short_message,
...
;';
            ...
            if (Mysqli::pwg_db_num_rows($result) > 0) {
                [$image_id] = Mysqli::pwg_db_fetch_row($result);
            }
        }
    ...
    ON ic.image_id = i.id
      AND i.level<=' . $userdata['level'];
        ...
        if (isset($filter_days)) {
            $query .= ' AND i.date_available > ' . Mysqli::pwg_db_get_recent_period_expression($filter_days);
        }
...
;';
        ...
        foreach ($cats as $idx => $cat) {
            ...
            $cats[$idx]['name'] = FunctionsPlugins::trigger_change('render_category_name', $cat['name'], $cat);
