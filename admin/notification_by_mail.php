...
function do_timeout_treatment(
    ...
    if ($env_nbm['is_sendmail_timeout'] && isset($_POST[$post_keyname])) {
        ...
        $time_refresh = $treated_count != 0 ? ceil(
            (get_moment() - $env_nbm['start_time']) * $post_count / $treated_count
        ) : 0;
    ...
    if (pwg_db_num_rows($result) > 0) {
        ...
        if ($env_nbm['is_sendmail_timeout']) {
            ...
            if (count($quoted_check_key_list) != 0) {
                $query = 'delete from ' . USER_MAIL_NOTIFICATION_TABLE . ' where check_key in (' . implode(
                    ',',
...
function render_global_customize_mail_content(
    ...
    if ($conf['nbm_send_html_mail'] && ! (str_starts_with((string) $customize_mail_content, '<'))) {
...
function do_action_send_mail_notification(
    ...
    if (in_array($action, ['list_to_send', 'send'])) {
        ...
        $data_users = get_user_notifications(
            'send',
            $check_key_list
        );
...
add_event_handler(
    ...
    'render_global_customize_mail_content'
);
    ...
    case 'subscribe':
        ...
        foreach ($data_users as $nbm_user) {
            if (get_boolean($nbm_user['enabled'])) {
                ...
                if ((isset($_POST['falsify']) && isset($_POST['cat_true']) && in_array(
                    $nbm_user['check_key'],
                    $_POST['cat_true']
                ))) {
                ...
                if (isset($_POST['trueify']) && isset($_POST['cat_false']) && in_array(
                    $nbm_user['check_key'],
                    $_POST['cat_false']
                )) {
