...
function do_timeout_treatment(
    ...
    if ($env_nbm['is_sendmail_timeout'] && isset($_POST[$post_keyname])) {
        ...
        $time_refresh = $treated_count != 0 ? ceil((get_moment() - $env_nbm['start_time']) * $post_count / $treated_count) : 0;
...
function insert_new_data_user_mail_notification(): void
{
    ...
    $query = "UPDATE users SET {$conf['user_fields']['email']} = NULL WHERE TRIM({$conf['user_fields']['email']}) = '';";
    pwg_query($query);
    ...
    "SELECT u.{$conf['user_fields']['id']} AS user_id, u.{$conf['user_fields']['username']} AS username, u.{$conf['user_fields']['email']} AS mail_address
     FROM users AS u LEFT JOIN user_mail_notification AS m ON u.{$conf['user_fields']['id']} = m.user_id WHERE u.{$conf['user_fields']['email']} IS NOT NULL AND
     m.user_id IS NULL ORDER BY user_id;";
    ...
    if (pwg_db_num_rows($result) > 0) {
        ...
        if ($env_nbm['is_sendmail_timeout']) {
            ...
            if (count($quoted_check_key_list) != 0) {
                ...
                $query = "DELETE FROM user_mail_notification WHERE check_key IN ({$quoted_check_key_list_});";
                $result = pwg_query($query);
        ...
        if (! $is_list_all_without_test || $is_action_send) {
            if ($data_users !== []) {
                ...
                foreach ($data_users as $nbm_user) {
                    ...
                    } elseif (news_exists($nbm_user['last_send'], $dbnow)) {
...
switch ($page['mode']) {
    ...
    case 'send':
        ...
        if ($data_users !== []) {
            foreach ($data_users as $nbm_user) {
                if (
                    ! $must_repost || $must_repost && in_array($nbm_user['check_key'], $_POST['send_selection'])  // Must be repost, show only user to send
                ) {
                    $tpl_var['users'][] =
                      [
