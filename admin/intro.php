...
if (! isset($_SESSION['cache_activity_last_weeks']) || $_SESSION['cache_activity_last_weeks']['calculated_on'] < strtotime(
  ...
  ;';
    ...
    $logger->debug(
        '[admin/intro::' . __LINE__ . '] recent activity calculated in ' . get_elapsed_time($start_time, get_moment())
    );
...
foreach ($file_extensions as $ext => $ext_details) {
    ...
    $data_storage[$type] = ($data_storage[$type] ?? null) + $ext_details['filesize'];
}
