...
function add_uploaded_file(
    ...
    if (! isset($image_id) && $conf['upload_detect_duplicate']) {
        $query = "SELECT id FROM images WHERE md5sum = '{$md5sum}';";
    ...
    if (isset($image_id)) {
        ...
        $query = "SELECT path FROM images WHERE id = {$image_id};";
    ...
    if (isset($image_id)) {
        ...
        $insert = [
            ...
            'path' => preg_replace('#^' . preg_quote(PHPWG_ROOT_PATH) . '#', '', (string) $file_path),
            'filesize' => $file_infos['filesize'],
            'width' => $file_infos['width'],
            'height' => $file_infos['height'],
            'md5sum' => $md5sum,
    ...
    // cache a derivative
    $query = "SELECT id, path, representative_ext FROM images WHERE id = {$image_id};";
...
function add_format(
    ...
    $query = "SELECT path FROM images WHERE id = {$format_of};";
...
function ready_for_upload_message(): string|null
{
    global $conf;
