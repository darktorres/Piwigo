 ...
 */
function delete_element_files(array $ids): int|array
{
  global $conf;
  if (count($ids) == 0)
  {
 ...
 */
function delete_elements(array $ids, bool $physical_deletion=false): int
{
  if (count($ids) == 0)
  {
    return 0;
 ...
 */
function update_category(array|int|string $ids = 'all')
{
  global $conf;
 ...
 * @return string[]
 */
function get_fs_directories($path, bool $recursive = true): array
{
  global $conf;
 ...
 * @return false|void
 */
function set_cat_visible(array $categories, bool|string $value, bool $unlock_child = false)
{
 ...
 * @return false|void
 */
function set_cat_status(array $categories, string $value)
{
 ...
 * @return string[]
 */
function get_fulldirs(array $cat_ids): array
{
  if (count($cat_ids) == 0)
  {
    return array();
 ...
 *
 */
function get_fs(string $path, bool $recursive = true): array
{
  global $conf;
 ...
 */
function move_categories(array $category_ids, int $new_parent = -1): void
{
  global $page;
 ...
 */
function create_virtual_category(string $category_name, int $parent_id=null, array $options=array()): array
{
  global $conf, $user;
 ...
 * @param int $image_id
 */
function set_tags(array $tags, int $image_id): void
{
 ...
 * @return int
 */
function tag_id_from_tag_name(string $tag_name): int
{
  global $page;
...
function compare_image_tag_lists(array $taglist_before, array $taglist_after): array
{
  ...
  foreach ($taglist_after as $image_id => $list_after)
  {
    ...
    $list_before = $taglist_before[$image_id] ?? array();
    sort($list_before);
 ...
 * @param int[] $categories
 */
function move_images_to_categories(array $images, array $categories)
{
  if (count($images) == 0)
  {
    return false;
 ...
 */
function associate_categories_to_categories(array $sources, array $destinations)
{
  if (count($sources) == 0)
  {
    return false;
 ...
 * @return string[]
 */
function get_extents(string $start=''): array
{
 ...
 * @return bool
 */
function cat_admin_access(int $category_id): bool
{
  global $user;
 ...
 * @return bool
 */
function fetchRemote(string $src, mixed &$dest, array $get_data=array(), array $post_data=array(), string $user_agent='Piwigo', int $step=0): bool
{
  global $conf;
 ...
 * @return string|false
 */
function get_groupname(int $group_id): false|string
{
  $query = '
SELECT name
  FROM '.GROUPS_TABLE.'
 ...
 * @return string|false
 */
function get_username(int $user_id): false|string
{
  global $conf;
 ...
 * @return string
 */
function get_newsletter_subscribe_base_url(string $language='en_UK'): string
{
 ...
 * @return int[]
 */
function get_tag_ids(array|string $raw_tags, bool $allow_create=true): array
{
  $tag_ids = array();
 ...
 */
function add_permission_on_category(array|int $category_ids, array|int $user_ids): void
{
 ...
 * @return int[]
 */
function get_admins(bool $include_webmaster=true): array
{
 ...
 * @param 'all'|int[] $types
 */
function clear_derivative_cache(array|string $types='all'): void
{
  if ($types === 'all')
  {
 ...
 * @param int|'all' $type
 */
function delete_element_derivatives(array $infos, int|string $type='all'): void
{
  $path = $infos['path'];
 ...
 * @return bool|void
 */
function deltree(string $path, string $trash_path=null)
{
  if (is_dir($path))
  {
    $fh = opendir($path);
 ...
 * @return string[]
 */
function get_admin_client_cache_keys(array|string $requested=array()): array
{
  $tables = array(
 ...
 */
function add_md5sum(array $ids): int
{
  $query = '
SELECT path
  FROM '.IMAGES_TABLE.'
 ...
 * @return void
 */
function save_images_order(int $category_id, array $images): void
{
  $current_rank = 0;
  $datas = array();
 ...
 * @return string
 */
function number_format_human_readable(float|null $numbers): string
{
  $readable = array("",  "k", "M");
  $index = 0;
