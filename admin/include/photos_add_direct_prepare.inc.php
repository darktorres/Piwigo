...
if (isset($_GET['album'])) {
    ...
    $query = "SELECT id, uppercats FROM categories WHERE id = {$_GET['album']};";
    ...
    // we need to know the category in which the last photo was added
    $query = 'SELECT category_id FROM images AS i JOIN image_category AS ic ON image_id = i.id JOIN categories AS c ON category_id = c.id ORDER BY i.id DESC LIMIT 1;';
    $result = pwg_query($query);
