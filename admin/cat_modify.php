...
$page['warnings'][] = l10n(
    'This album is currently locked, visible only to administrators.'
) . '<span class="icon-key unlock-album">' . l10n(
    'Unlock it'
) . '</span>';
...
$template->assign(
    [
        ...
        'CATEGORIES_PARENT_NAV' => preg_replace(
            '# {2,}#',
            ' ',
            preg_replace("#(\r\n|\n\r|\n|\r)#", ' ', $parent_navigation)
        ),
...
$template->assign(
    [
        ...
        'INFO_LAST_MODIFIED_SINCE' => time_since(
...
if ($category['has_images'] || ! empty($category['representative_picture_id'])) {
    ...
    if (! empty($category['representative_picture_id'])) {
        $tpl_representant['picture'] = get_category_representant_properties(
            $category['representative_picture_id'],
            IMG_SMALL
        );
    }
