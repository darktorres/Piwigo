...
if (! is_webmaster()) {
    $page['warnings'][] = str_replace(
        ...
        l10n('%s status is required to edit parameters.')
    );
}
            ...
            if (! isset($conf['order_by_custom']) && ! isset($conf['order_by_inside_category_custom'])) {
                if (! empty($_POST['order_by'])) {
                    check_input_parameter(
                        ...
                        $_POST,
                        true,
                        '/^(' . implode('|', array_keys($sort_fields)) . ')$/'
                    );
                    ...
                    if (! count($_POST['order_by'])) {
                        ...
                        $order_by = $order_by_inside_category = array_slice(
                            $_POST['order_by'],
                            0,
                            (int) ceil(count($sort_fields) / 2)
                        );
...
$template->set_filename(
    'config',
    'configuration_' . $page['section'] . '.tpl'
);
...
switch ($page['section']) {
    case 'main':
        ...
        if (order_by_is_local()) {
            $page['warnings'][] = l10n(
                'You have specified <i>$conf[\'order_by\']</i> in your local configuration file, this parameter in deprecated, please remove it or rename it into <i>$conf[\'order_by_custom\']</i> !'
            );
        }
        ...
        $template->assign(
            ...
            [
                ...
                'email_admin_on_new_user_filter' => in_array(
