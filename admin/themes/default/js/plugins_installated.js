...
function activatePlugin(id) {
  ...
  $.ajax({
    ...
    dataType: "json",
    url: "ws.php",
    data: { method: "pwg.plugins.performAction", action: "activate", plugin: id, pwg_token: pwg_token, format: "json" },
  ...
  $.alert({
    ...
    content: function () {
      return $.ajax({
        ...
        dataType: "json",
        url: "ws.php",
        data: { method: "pwg.plugins.performAction", action: "delete", plugin: id, pwg_token: pwg_token, format: "json" },
        success: function (data) {
          if (data.stat === "ok") {
            $("#" + id).remove();
            nb_plugin.inactive -= 1;
...
function restorePlugin(id) {
  $.ajax({
    ...
    dataType: "json",
    url: "ws.php",
    data: { method: "pwg.plugins.performAction", action: "restore", plugin: id, pwg_token: pwg_token, format: "json" },
    success: function (data) {
      if (data.stat == "ok") {
        let pluginName = id;
        $("#" + id + " .pluginNotif").stop(false, true);
...
function uninstallPlugin(id) {
  $.ajax({
    ...
    dataType: "json",
    url: "ws.php",
    data: { method: "pwg.plugins.performAction", action: "uninstall", plugin: id, pwg_token: pwg_token, format: "json" },
    success: function (data) {
      $("#" + id).remove();
      nb_plugin.other -= 1;
      nb_plugin.all -= 1;
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      for (i = 0; i < data.length; i++) {
        if (show_details) jQuery(`#${data[i]} .pluginName`).prepend(`<a class="warning" title="${incompatible_msg}"></a>`);
        else jQuery(`#${data[i]} .pluginName`).prepend(`<span class="warning" title="${incompatible_msg}"></span>`);
