...
var toogleSelection = function (group_id, toggle) {
  ...
  if (toggle) {
    ...
    item = $("<div data-id=" + group_id + ">" + "<a class='icon-cancel'></a>" + "<p>" + groupBox.find("#group_name").html() + "</p> </div>");
  ...
  jQuery.ajax({
    ...
    success: function (raw_data) {
      ...
      if (data.stat === "ok") {
        ...
        $.alert({
          ...{ title: str_merged_into.replace("%s1", name_merge.toString()).replace("%s2", name_dest), content: "" },
          ...jConfirm_alert_options,
        });
  ...
  jQuery.ajax({
    ...
    success: function (raw_data) {
      ...
      if (data.stat === "ok") {
        ...
        $.alert({
          ...{ title: str_groups_deleted.replace("%s", names.toString()), content: "" },
          ...jConfirm_alert_options,
        });
        updateBadge();
      }
...
var dissociateUserInfo = $("<div class='ValidationUserDissociated'>" + "<p class='icon-ok'></p>" + "</div>")
  .appendTo(".group-name-block")
  .hide();

var associateUserInfo = $("<div class='ValidationUserAssociated'>" + "<p class='icon-ok'></p>" + "</div>");
...
function updateMembernumber(number, grp_id) {
  $(".GroupContainer[data-id=" + grp_id + "] .group_number_users").html(number + " " + (number > 1 ? str_members_default : str_member_default));
