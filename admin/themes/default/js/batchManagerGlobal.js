    ...
    (function (ids) {
      ...
      queuedManager.add({
        ...
        success: function (data) {
          ...
          /*TODO: user feedback only data.result images out of thisBatchSize were deleted*/ /*TODO: user feedback if isError*/
          jQuery("#regenerationStatus .badge-number").html(todo.toString() + "/" + progressBar_max.toString());
          progress_bar(todo, progressBar_max, false);
        },
        error: function (data) {
...
jQuery("#sync_md5sum").on("click", function (e) {
  ...
  var addBlockSize = Math.min(Number((jQuery("#md5sum_to_add").data("origin") / 2).toFixed()), 1000);
...
function add_md5sum_block(blockSize) {
  jQuery.ajax({
    ...
    success: function (data) {
      ...
      var percent_remaining = Number(((data.result.nb_no_md5sum * 100) / jQuery("#md5sum_to_add").data("origin")).toFixed());
    ...
    error: function (XMLHttpRequest) {
      ...
      jQuery("#add_md5sum_error")
        .show()
        .html("error " + XMLHttpRequest.status + " : " + XMLHttpRequest.statusText);
    },
  });
}
...
jQuery("#delete_orphans").on("click", function (e) {
  ...
  var deleteBlockSize = Math.min(Number((jQuery("#orphans_to_delete").data("origin") / 2).toFixed()), 1000);
...
function delete_orphans_block(blockSize) {
  jQuery.ajax({
    ...
    success: function (data) {
      ...
      var percent_remaining = Number(((data.result.nb_orphans * 100) / jQuery("#orphans_to_delete").data("origin")).toFixed());
      var percent_done = 100 - percent_remaining;
