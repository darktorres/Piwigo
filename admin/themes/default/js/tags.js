...
$(".TagSubmit").on("click", function () {
  $(".TagSubmit").hide();
  $(".TagLoading").show();
  renameTag($(".RenameTagPopInContainer").find(".tag-property-input").attr("id"), $(".RenameTagPopInContainer").find(".tag-property-input").val())
    .then(() => {
      $(".TagSubmit").show();
      $(".TagLoading").hide();
      rename_tag_close();
...
function updateMergeItems() {
  ...
  selected.forEach((id) => {
    $("#MergeOptionsChoices").append($(`<option value="${id}">${dataTags.find((tag) => tag.id == id).name}</option>`));
  });
}
...
$("#selectAll").on("click", function () {
  ...
  if (selected.length < dataTags.length) {
    showSelectMessage(str_selection_done.replace("%d", $(".tag-box").length), str_select_all_tag.replace("%d", dataTags.length), function () {
        ...
        success: function (raw_data) {
          ...
          if (data.stat === "ok") {
            ...
            if (data.result.images_in_merged_tag.length > 0) {
              ...
              tagBox.find(".dropdown-option.view," + ".dropdown-option.manage," + ".tag-dropdown-header i").show();
...
function tagListToString(list) {
  if (list.length > 5) {
    return list.slice(0, 5).join(", ") + " " + str_and_others_tags.replace("%s", list.length - 5);
  } else {
    return list.join(", ");
  }
}
