  ...
  $("#cat-properties-save").click(() => {
    ...
    if (parent_album != default_parent_album) {
      jQuery.ajax({
        ...
        success: function (data) {
          if (data.stat === "ok") {
            $(".cat-modify-ariane").html(data.result.new_ariane_string);
      ...
      content: function () {
        ...
        return $.ajax({
          ...
          success: function (raw_data) {
            ...
            let message = `<p>
              ${str_delete_album_and_his_x_subalbums.replace("%s", `<strong>${album_name}</strong>`).replace("%d", `<strong>${nb_sub_albums}</strong>`)}
              </p>`;
            ...
            if (data.nb_images_recursive) {
              ...
              message += `<div> 
                ...
                <label for="force_delete">${str_delete_all_photos.replaceAll(
                  "%d",
                  (_) => [data.nb_images_recursive, data.nb_images_associated_outside][t++]
                )}</label>
              </div>`;
            }
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        ...
        if (data.stat == "ok") {
          ...
          jQuery(".cat-modify-representative").attr("style", `background-image:url('${data.result.src}')`).removeClass("icon-dice-solid");
        } else {
          console.error(data);
        }
  ...
  cats.forEach((cat) => {
    $("#searchResult").append(
      `<div class='search-result-item' id="${cat.id}">
        ...
        <span class='icon-plus-circled item-add'></span>
      </div>`
    );
