...
function createAlbumNode(node, li) {
  ...
  if (node.children.length != 0) {
    ...
    cont.append($(toggler_cont.replace(/%content%/g, toggler).replace(/%id%/g, node.id)));
  } else {
    cont.find(".move-cat-order").addClass("notClickable");

    cont.append($(toggler_cont.replace(/%content%/g, toggler_close).replace(/%id%/g, node.id))).addClass("disabledToggle");
  }
      ...
      if (data.nb_images_recursive == 0) {
        ...
        if (data.nb_images_associated_outside == 0) {
          ...
          $("#IMAGES_ASSOCIATED_OUTSIDE .innerText").append(
            has_images_associated_outside.replace("%d", data.nb_images_recursive).replace("%d", data.nb_images_associated_outside)
          );
        }
