...
SELECT
  ...
  WHERE ' . implode(' AND ', $clauses);
        ...
        while ($file = readdir($dir)) {
            if ($file != '.' and $file != '..') {
                ...
                if (is_dir($path)
                    ...
                    if ($desc = \Piwigo\inc\functions::load_language('description.txt', $path . '/', [
                        'return' => true,
                    ])) {
    ...
    public function sort_fs_themes($order = 'name')
    {
        switch ($order) {
            case 'name':
                uasort($this->fs_themes, '\Piwigo\inc\functions_html::name_compare');
                break;
            case 'status':
            ...
            if ($handle = @fopen($archive, 'wb') and functions::fetchRemote($url, $handle, $get_data)) {
                ...
                if ($list = $zip->listContent()) {
                    foreach ($list as $file) {
                        ...
                        if (basename($file['filename']) == 'themeconf.php'
