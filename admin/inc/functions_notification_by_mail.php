...
$env_nbm = array
          (
            'start_time' => \Piwigo\inc\functions::get_moment(),
...
class functions_notification_by_mail
{
  ...
  */
  static function check_sendmail_timeout()
  {
    ...
    $env_nbm['is_sendmail_timeout'] = ((\Piwigo\inc\functions::get_moment() - $env_nbm['start_time']) > $env_nbm['sendmail_timeout']);
  ...
  */
  static function begin_users_env_nbm($is_to_send_mail = false)
  {
    ...
    if ($is_to_send_mail)
    {
      ...
      $env_nbm['send_as_mail_address'] = \Piwigo\inc\functions::get_webmaster_mail_address();
      ...
      $env_nbm['msg_info'] = \Piwigo\inc\functions::l10n('Mail sent to %s [%s].');
      $env_nbm['msg_error'] = \Piwigo\inc\functions::l10n('Error when sending email to %s [%s].');
    }
  }
  ...
  */
  static function display_counter_info()
  {
    ...
    {
      $page['errors'][] = \Piwigo\inc\functions::l10n_dec(
    ...
    else
    {
      if ($env_nbm['sent_mail_count'] == 0)
      {
        $page['infos'][] = \Piwigo\inc\functions::l10n('No mail to send.');
      }
      else
      {
        $page['infos'][] = \Piwigo\inc\functions::l10n_dec(
  ...
  */
  static function do_subscribe_unsubscribe_notification_by_mail($is_admin_request, $is_subscribe = false, $check_key_list = array())
  {
    ...
    if ($is_subscribe)
    {
      $msg_info = \Piwigo\inc\functions::l10n('User %s [%s] was added to the subscription list.');
      $msg_error = \Piwigo\inc\functions::l10n('User %s [%s] was not added to the subscription list.');
    }
    else
    {
      $msg_info = \Piwigo\inc\functions::l10n('User %s [%s] was removed from the subscription list.');
      $msg_error = \Piwigo\inc\functions::l10n('User %s [%s] was not removed from the subscription list.');
    }
    ...
    if (count($check_key_list) != 0)
    {
      ...
      $msg_break_timeout = \Piwigo\inc\functions::l10n('Time to send mail is limited. Others mails are skipped.');
      ...
      foreach ($data_users as $nbm_user)
      {
        ...
        if ($nbm_user['mail_address'] != '')
        {
          ...
          $subject = '['.$conf['gallery_title'].'] '.($is_subscribe ? \Piwigo\inc\functions::l10n('Subscribe to notification by mail'): \Piwigo\inc\functions::l10n('Unsubscribe from notification by mail'));
