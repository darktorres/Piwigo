...
class updates
{
    ...
    public static function check_piwigo_upgrade()
    {
        ...
        if (preg_match('/(\d+\.\d+)\.(\d+)/', PHPWG_VERSION, $matches)
          and @functions::fetchRemote(PHPWG_URL . '/download/all_versions.php?rand=' . md5(uniqid(mt_rand(), true)), $result)) {
            ...
            if (@filesize($filename)) {
                ...
                if ($result = $zip->extract(
                    ...
                    if (empty($error)) {
                        ...
                        \Piwigo\inc\functions::pwg_activity('system', ACTIVITY_SYSTEM_CORE, 'update', [
