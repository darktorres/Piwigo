...
class check_integrity
{
    ...
    public function check(): void
    {
        ...
        if (
            is_array(
                $conf_c13y_ignore
            ) && isset($conf_c13y_ignore['version']) && $conf_c13y_ignore['version'] == PHPWG_VERSION && is_array(
                $conf_c13y_ignore['list']
            )
        ) {
        ...
        if (isset($_POST['c13y_submit_correction']) && isset($_POST['c13y_selection'])) {
            ...
            foreach ($this->retrieve_list as $i => $c13y) {
                if (! empty($c13y['correction_fct']) && $c13y['is_callable'] && in_array(
                    $c13y['id'],
                    $_POST['c13y_selection']
                )) {
        ...
        $ignore_list_changed =
          (
              $ignore_list_changed || array_diff($this->ignore_list, $this->build_ignore_list) !== [] || array_diff(
                  $this->build_ignore_list,
                  $this->ignore_list
              ) !== []
          );
    ...
    public function display(): void
    {
        ...
        if (isset($this->retrieve_list) && $this->retrieve_list !== []) {
    ...
    public function add_anomaly(
        string $anomaly,
        string|null $correction_fct = null,
        string|null $correction_fct_args = null,
        string|null $correction_msg = null
    ): void {
