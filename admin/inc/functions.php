...
class functions
{
  ...
  DELETE FROM ' . USER_CACHE_CATEGORIES_TABLE . '
    WHERE cat_id IN (' . implode(',', $ids) . ')';
        ...
        \Piwigo\inc\functions::pwg_activity('album', $ids, 'delete', [
  ...
  ;';
            ...
            usort($all_categories, '\Piwigo\inc\functions_category::global_rank_compare');
  ...
  ;';
    ...
    public static function get_category_representant_properties($image_id, $size = null)
    {
        $query = '
  ...
  ;';
        ...
        \Piwigo\inc\functions::pwg_activity('album', $category_ids, 'move', [
            'parent' => $new_parent,
        ]);
    }
    ...
    public static function create_virtual_category($category_name, $parent_id = null, $options = [])
    {
        ...
        if (preg_match('/^\s*$/', $category_name)) {
            return [
                'error' => \Piwigo\inc\functions::l10n('The name of an album must not be empty'),
            ];
        }
  ...
  ;';
      ...
      ;';
        ...
        \Piwigo\inc\functions_plugins::trigger_notify('create_virtual_category', array_merge([
            'id' => $inserted_id,
        ], $insert));
  ...
  ;';
            if (count($existing_tags = functions_mysqli::query2array($query, null, 'id')) == 0) {
                ...
                $sub_name_where = \Piwigo\inc\functions_plugins::trigger_change('get_tag_name_like_where', [], $tag_name);
    ...
    public static function get_taglist($query, $only_user_language = true)
    {
        ...
        while ($row = functions_mysqli::pwg_db_fetch_assoc($result)) {
            ...
            if (! $only_user_language) {
                $alt_names = \Piwigo\inc\functions_plugins::trigger_change('get_tag_alt_names', [], $raw_name);
        ...
        usort($taglist, '\Piwigo\inc\functions_html::tag_alpha_compare');
        if (count($altlist)) {
            usort($altlist, '\Piwigo\inc\functions_html::tag_alpha_compare');
  ...
  ;';
    ...
    {
        if (\Piwigo\inc\functions::conf_get_param('count_orphans') === null) {
