...
function quote_check_key_list(
    ...
    return array_map(static fn ($s) => "'" . $s . "'", $check_key_list);
}
...
function do_subscribe_unsubscribe_notification_by_mail(
    ...
    if (count($check_key_list) != 0) {
        ...
        $msg_break_timeout = l10n(
            'Time to send mail is limited. Others mails are skipped.'
        );
        ...
        foreach ($data_users as $nbm_user) {
            ...
            if ($nbm_user['mail_address'] != '') {
                ...
                $subject = '[' . $conf['gallery_title'] . '] ' . ($is_subscribe ? l10n(
                    'Subscribe to notification by mail'
                ) : l10n(
                    'Unsubscribe from notification by mail'
                ));
