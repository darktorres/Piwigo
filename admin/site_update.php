  ...
  foreach (array_diff($fs_fulldirs, array_keys($db_fulldirs)) as $fulldir)
  {
    ...
    if (preg_match($conf['sync_chars_regex'], $dir))
    {
      ...
      if (isset($db_fulldirs[dirname($fulldir)]))
      {
        ...
        $insert['rank_column'] = $next_rank[$parent]++;
        $insert['global_rank'] =
          $db_categories[$parent]['global_rank'].'.'.$insert['rank_column'];
      ...
      else
      {
        ...
        $insert['rank_column'] = $next_rank['NULL']++;
        $insert['global_rank'] = $insert['rank_column'];
      }
  ...
  if (count($inserts) > 0)
  {
    if (!$simulate)
    {
      ...
      if ($conf['inheritance_by_default'] and !empty($category_up))
      {
        ...
        mass_inserts('group_access', array('group_id','cat_id'), $insert_granted_grps);
        $insert_granted_users=array_unique($insert_granted_users, SORT_REGULAR);
        mass_inserts('user_access', array('user_id','cat_id'), $insert_granted_users);
      }
      else
      {
