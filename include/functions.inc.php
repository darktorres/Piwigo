...
function str2DateTime(
    ...
    if ($format !== null && $format !== '' && $format !== '0' && version_compare(
        PHP_VERSION,
        '5.3.0'
    ) >= 0) {// from known date format
...
SELECT param, value
 FROM ' . CONFIG_TABLE . '
 ' . ($condition === '' || $condition === '0' ? '' : 'WHERE ' . $condition) . '
;';
    $result = pwg_query($query);
...
function load_language(
    ...
    if ($dirname !== '' && $dirname !== '0' && ($filename !== '' && $filename !== '0') && ! ($options['return'] ?? false)
...
function create_navigation_bar(
    ...
    if ($nb_element > $nb_element_page) {
        ...
        $navbar['pages'] = [];
        $navbar['pages'][1] = $url;
        for ($i = max(floor($cur_page) - $pages_around, 2), $stop = min(
            ceil($cur_page) + $pages_around + 1,
            $maximum
        );
            $i < $stop; $i++) {
