...
class PHPMailer
{
    ...
    public function isSendmail(): void
    {
        ...
        $this->Sendmail = stripos(
            $ini_sendmail_path,
            'sendmail'
        ) === false ? '/usr/sbin/sendmail' : $ini_sendmail_path;
    ...
    public function isQmail(): void
    {
        ...
        $this->Sendmail = stripos(
            $ini_sendmail_path,
            'qmail'
        ) === false ? '/var/qmail/bin/qmail-inject' : $ini_sendmail_path;
    ...
    public function preSend()
    {
        ...
        try {
            ...
            foreach ([
                ...
                if ($this->{$address_kind} === '' || $this->{$address_kind} === '0') {
                    continue;
                }
     ...
     * @return SMTP
     */
    public function setSMTPInstance(
        SMTP $smtp
    ) {
        $this->smtp = $smtp;
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ($this->smtp->connect($prefix . $host, $port, $this->Timeout, $options)) {
                try {
                    ...
                    if ($this->SMTPAutoTLS && $sslext && $secure !== 'ssl' && $this->smtp->getServerExt(
                        'STARTTLS'
                    )) {
                        $tls = true;
                    }
    ...
    public function setLanguage(
        ...
        if ($langcode !== 'en') {
            $langcodes = [];
            if (! empty($matches['script']) && (isset($matches['country']) && ($matches['country'] !== '' && $matches['country'] !== '0'))) {
    ...
    public function createBody(): string
    {
        ...
        if (static::ENCODING_8BIT === $bodyEncoding && ! $this->has8bitChars(
            $this->Body
        )) {
        ...
        if (static::ENCODING_BASE64 !== $altBodyEncoding && static::hasLineLongerThanMax(
            $this->AltBody
        )) {
        ...
        switch ($encoding) {
            case 'B':
                if ($this->hasMultiBytes($str)) {
                    ...
                    $encoded = $this->base64EncodeWrapMB(
                        $str,
                        "\n"
                    );
                } else {
                ...
                $encoded = preg_replace('/^(.*)$/m', ' =?' . $charset . sprintf('?%s?\1?=', $encoding), $encoded);
                break;
            case 'Q':
    ...
    public function encodeQ(
        ...
        $matches = [];
        if (preg_match_all(sprintf('/[%s]/', $pattern), $encoded, $matches)) {
    ...
    public function clearQueuedAddresses(
        ...
        $this->RecipientsQueue = array_filter(
            $this->RecipientsQueue,
            static fn ($params): bool => $params[0] !== $kind
        );
    }
     ...
     */
    public static function isValidHost(
        $host
    ): bool {
        ...
        if (strlen($host) > 2 && str_starts_with($host, '[') && str_ends_with(
            $host,
            ']'
        )) {
    ...
    public function msgHTML(
        ...
        $this->Body = static::normalizeBreaks(
            $message
        );
     ...
     */
    public static function stripTrailingWSP(
        $text
    ): string {
        return rtrim($text, " \r\n\t");
    }
    ...
    public function DKIM_HeaderC(
        ...
        $signHeader = static::normalizeBreaks(
            $signHeader,
            self::CRLF
        );
        //Unfold header lines
    ...
    protected function edebug(
        ...
        if (is_callable($this->Debugoutput) && ! in_array(
            $this->Debugoutput,
            ['error_log', 'html', 'echo']
        )) {
    ...
    protected function addOrEnqueueAnAddress(
        ...
        if (static::idnSupported() && $this->has8bitChars(
            substr($address, ++$pos)
        )) {
            if ($kind !== 'Reply-To') {
        ...
        return $this->addAnAddress(...$params);
    }
    ...
    protected function sendmailSend(
        ...
        if (! empty($this->Sender) && static::validateAddress(
            $this->Sender
        ) && self::isShellSafe(
            $this->Sender
        )) {
    ...
    protected static function isShellSafe(
        ...
        if (
            ...
            || ! in_array(escapeshellarg($string), [sprintf("'%s'", $string), sprintf('"%s"', $string)])
        ) {
            return false;
        }
     ...
     */
    protected static function isPermittedPath(
        $path
    ): bool {
    ...
    protected function smtpSend(
        ...
        if ((count($this->all_recipients) > count(
            $bad_rcpt
        )) && ! $this->smtp->data(
            $header . $body
        )) {
    ...
    protected function attachAll(
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] === $disposition_type) {
                ...
                $mime[] = $bString ? $this->encodeString(
                    $string,
                    $encoding
                ) : $this->encodeFile(
                    $path,
     ...
     * @param string $le
     */
    protected static function setLE(
        $le
    ) {
        static::$LE = $le;
    }
