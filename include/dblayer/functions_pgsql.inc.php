...
function pwg_db_real_escape_string(string|int|null $s): string|null
{
    global $pg;
...
function mass_updates(
    ...
    if (count($datas) < 10) {
        ...
        if (($flags & MASS_UPDATES_SKIP_EMPTY) !== 0) {
            $func_set = fn (string $s): string => "t1.{$s} = COALESCE(t2.{$s}, t1.{$s})";
        } else {
        ...
        $query = "
            UPDATE {$tablename} t1
            SET {$dbfields_update_}
            FROM {$temporary_tablename} t2
            WHERE {$dbfields_primary_};
        ";
        pwg_query($query);
