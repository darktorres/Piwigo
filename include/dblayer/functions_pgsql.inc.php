...
function pwg_db_connect(
    ...
    $dbname = $database === null || $database === '' || $database === '0' ? '' : "dbname={$database}";
    $connection_string = "host={$host} port={$port} {$dbname} user={$user} password={$password}";
 ...
 * Execute a query
 */
function pwg_query(string $query): PgSql\Result|bool
{
...
function pwg_db_num_rows($result): int|string
{
    return pg_num_rows($result);
}

function pwg_db_fetch_array($result): array|bool|null
{
    return pg_fetch_array($result, null, PGSQL_BOTH);
}

function pwg_db_fetch_assoc($result): array|bool|null
{
    return pg_fetch_assoc($result);
}

function pwg_db_fetch_row($result): array|bool|null
{
    return pg_fetch_row($result);
}

function pwg_db_fetch_object($result): bool|object|null
{
    return pg_fetch_object($result);
}

function pwg_db_free_result($result): void
{
    pg_free_result($result);
}

function pwg_db_real_escape_string(string|int $s): string|null
{
    global $pg;
...
UPDATE {$tablename}
  ...
  WHERE ';
        ...
        $result = pwg_query(
         ...
         WHERE table_name = \'' . pwg_db_real_escape_string($tablename) . "'"
        );
        $columns = [];
...
(
' . implode(",\n  ", $columns) . ',
UNIQUE (' . implode(',', $dbfields['primary']) . ')
)';
        ...
        if (($flags & MASS_UPDATES_SKIP_EMPTY) !== 0) {
            $func_set = fn (string $s): string => sprintf('t1.%s = COALESCE(t2.%s, t1.%s)', $s, $s, $s);
        } else {
            $func_set = fn (string $s): string => sprintf('t1.%s = t2.%s', $s, $s);
        }

        $query .= '
    UPDATE ' . $tablename . ' t1
      SET ' . implode(",\n    ", array_map($func_set, $dbfields['update'])) . '
      FROM ' . $temporary_tablename . ' t2
      WHERE ' . implode(' AND ', array_map(fn ($key): string => sprintf('t1.%s = t2.%s', $key, $key), $dbfields['primary'])) . '
      ';
...
function pwg_db_get_recent_period(int|string $period, string $date = 'CURRENT_DATE'): string
{
    $query = '
SELECT ' . pwg_db_get_recent_period_expression($period) . ' AS date';
 ...
 * error occured for the last PostgreSQL query.
 */
function my_error(
    string $header,
    bool $die
    ...
    $error = '[pgsql error] ' . pg_last_error($pgsql) . "\n";
    $error .= $header;
