...
function insert_user_comment(
    ...
    if (! is_classic_user()) {
        ...
        if ($comm['author'] != 'guest') {
            $author_ = addslashes($comm['author']);
    ...
    if ($comment_action != 'reject') {
        ...
        $website_url_ = (! empty($comm['website_url']) ? '\'' . $comm['website_url'] . '\'' : 'NULL');
        $email_ = (! empty($comm['email']) ? '\'' . $comm['email'] . '\'' : 'NULL');
        $query =
...
function update_user_comment(
    ...
    if ($comment_action != 'reject') {
        ...
        $website_url_ = (! empty($comment['website_url']) ? '\'' . $comment['website_url'] . '\'' : 'NULL');
        $validate_ = ($comment_action == 'validate' ? 'true' : 'false');
        $validate_2 = ($comment_action == 'validate' ? 'NOW()' : 'NULL');
        $query =
