...
class Smarty_Internal_Compile_Include extends Smarty_Internal_CompileBase
{
    ...
    public function compile(
        ...
        $fullResourceName = $_attr['file'];
        $source_resource = $_attr['file'];
        ...
        if ($merge_compiled_includes) {
            ...
            // variable compile_id?
            if (isset($_attr['compile_id']) && $compiler->isVariable(
                $_attr['compile_id']
            )) {
                $merge_compiled_includes = false;
            }
        ...
        */
        if ($_attr['nocache'] !== true && $_attr['caching']) {
            $_caching = (int) $_attr['caching'];
            $_new_caching = (int) $_attr['caching'];
            $call_nocache = true;
        } else {
        ...
        if (isset($_attr['assign'])) {
            ...
            if ($_assign = $compiler->getId(
                $_attr['assign']
            )) {
                $_assign = sprintf("'%s'", $_assign);
    ...
    public function compileInlineTemplate(
        ...
        if (! ($tpl->source->handler->uncompiled) && $tpl->source->exists) {
            ...
            // get compiled code
            $compiled_code = "<?php\n\n";
            $compiled_code .= $compiler->cStyleComment(
                sprintf(' Start inline template "%s" =============================', $sourceInfo)
            ) . "\n";
            ...
            $compiled_code .= "<?php\n\n";
            $compiled_code .= $compiler->cStyleComment(
                sprintf(' End inline template "%s" =============================', $sourceInfo)
            ) . "\n";
            $compiled_code .= '?>';
            unset($tpl->compiler);
