...
abstract class Smarty_Internal_TemplateCompilerBase
{
    ...
    public function compileVariable(
        ...
        if (! strpos($variable, '(')) {
            ...
            $var = trim($variable, "'");
            $this->tag_nocache |= $this->template->ext->getTemplateVars->_getVariable(
                $this->template,
                $var,
                null,
                true,
    ...
    public function compilePHPFunctionCall(
        ...
        if (! $this->smarty->security_policy || $this->smarty->security_policy->isTrustedPhpFunction($name, $this)) {
            if (strcasecmp($name, 'isset') === 0 || strcasecmp($name, 'empty') === 0
                ...
                if ($func_name === 'isset') {
                    ...
                    if (count($parameter) !== 1) {
                        $this->trigger_template_error(sprintf("Illegal number of parameter in '%s'", $func_name()));
                    }
            ...
            $this->trigger_template_error(sprintf("unknown function '%s'", $name));
    ...
    public function callTagCompiler(
        ...
        // compile this tag
        return $tagCompiler === false ? false : $tagCompiler->compile(
            $args,
            $this,
            $param1,
            $param2,
     ...
     */
    public function getTagCompiler(
        $tag
    ) {
        ...
        if (! isset(self::$_tag_objects[$tag])) {
            ...
            if (class_exists($class_name)
                && ($this->smarty->security_policy === null || $this->smarty->security_policy->isTrustedTag(
                    $tag,
                    $this
                ))
            ) {
     ...
     */
    public function getPluginFromDefaultHandler(
        $tag,
        $plugin_type
    ) {
        $callback = null;
        ...
        if ($result) {
            ...
            if ($script !== null) {
                if (is_file($script)) {
                    ...
                    $this->trigger_template_error(
                        sprintf("Default plugin handler: Returned script file '%s' for '%s' not found", $script, $tag)
                    );
                }
            }
            ...
            $this->trigger_template_error(
                sprintf("Default plugin handler: Returned callback for '%s' not callable", $tag)
            );
    ...
    public function getId(
        ...
        if (preg_match('~^([\'"]*)(\d*[a-zA-Z_]\w*)\1$~', $input, $match)) {
            return $match[2];
        }
    ...
    public function getVariableName(
        ...
        if (preg_match('~^[$]_smarty_tpl->tpl_vars\[[\'"]*(\d*[a-zA-Z_]\w*)[\'"]*\]->value$~', $input, $match)) {
            return $match[1];
        }
    ...
    public function convertScope(
        ...
        if (isset($_attr['scope'])) {
            ...
            if ($_scope === false) {
                ...
                $this->trigger_template_error(
                    sprintf('illegal value \'%s\' for "scope" attribute', $err),
                    null,
                    true
                );
            }
     ...
     * @return string
     */
    public function makeNocacheCode(
        $code
    ) {
        return sprintf("echo '/*%%%%SmartyNocache:%s%%%%*/<?php ", $this->nocache_hash) .
    ...
    public function leaveDoubleQuote()
    {
        if (array_pop($this->_tag_stack_count) !== $this->getTagStackCount()) {
            ...
            $this->trigger_template_error(
                sprintf("unclosed '{%s}' in doubled quoted string", $tag),
                null,
                true
            );
        }
     ...
     * @return string
     */
    public function compileCheckPlugins(
        $requiredPlugins
    ) {
    ...
    private function compileTag2(
        ...
        if (($_output = $this->callTagCompiler(
            ...
            $args,
            $parameter
        )) === false && (isset($this->parent_compiler->tpl_function[$tag])
            || ($this->template->smarty->ext->_tplFunction !== null
                && $this->template->smarty->ext->_tplFunction->getTplFunction($this->template, $tag) !== false))) {
            // template defined by {template} tag
            $args['_attr']['name'] = sprintf("'%s'", $tag);
        ...
        if (strlen($tag) < 6 || ! str_ends_with($tag, 'close')) {
            ...
            foreach ($this->plugin_search_order as $plugin_type) {
                if ($plugin_type === Smarty::PLUGIN_COMPILER
                    ...
                    throw new SmartyException(sprintf("Plugin '%s' not callable", $tag));
                } elseif ($function = $this->getPlugin($tag, $plugin_type)) {
                    if ($this->smarty->security_policy === null
                        || $this->smarty->security_policy->isTrustedTag($tag, $this)
                    ) {
                        return $this->callTagCompiler(
        ...
        $this->trigger_template_error(sprintf("unknown tag '%s'", $tag), null, true);
        return null;
